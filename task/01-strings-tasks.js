...
function removeFirstOccurrences(str, value) {
    return str.substring(0, str.indexOf(value)).concat(str.substr(str.indexOf(value) + value.length))
...
function getRectangleString(width, height) {
    ...
    var vertical = '│';
    var result = "";
    result = result.concat(upperLeft, repeatString(horizontal, width - 2), upperRight);
    for (var i = 0; i < height - 2; i++) {
        result = result.concat("\n", vertical, repeatString(" ", width - 2), vertical)
    }
    result = result.concat("\n", downsideLeft, repeatString(horizontal, width - 2), downsideRight + "\n");
    return result;
}
function repeatString(value, count) {
    return value.repeat(count);
